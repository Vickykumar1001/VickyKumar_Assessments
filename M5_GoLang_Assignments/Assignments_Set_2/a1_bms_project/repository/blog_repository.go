...
func (repo *BlogRepository) GetBlogByID(id int) (*model.Blog, error) {
...
func (repo *BlogRepository) GetAllBlogs() ([]model.Blog, error) {
	rows, err := repo.DB.Query("SELECT id, title, content, author, timestamp FROM blogs ORDER BY timestamp DESC")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
